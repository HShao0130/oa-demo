...
//   async find(reqAccount, reqPassword) {
//     const { app } = this;
//     const user = await app.mysql.get('user', { account: reqAccount, password: reqPassword });
//     console.log('user.ts:', JSON.parse(JSON.stringify(user)));
...
export default class UserService extends Service {
   ...
   */
  public async getUserByUserId(reqUserId) {
  ...
  public async getUserList(userId) {
    const { app } = this;
    console.log('op', app.sequelize.Op);
   ...
   * @param  email // 邮箱
   */
  private async hasRegister(reqEmail) {
    // 查询用户名
